...
Original file is located at
    https://colab.research.google.com/drive/1gnUhWSawp-zjLIKh9h637fDqa7c5_QAv
"""
...
# 数値型の欠損値を平均値で補完する
# phys_cols = ['Age', 'Sprint_40yd', 'Vertical_Jump', 'Bench_Press_Reps',
#                 'Broad_Jump', 'Agility_3cone', 'Shuttle']

# # 1) 先基于 train 计算各位置的中位数（也可改成 .mean() 取平均）
# pos_medians = train.groupby("Position")[phys_cols].median()

# # 2) 在 train 中按位置用中位数填补缺失
# for col in phys_cols:
#     train[col] = train.groupby("Position")[col].transform(lambda s: s.fillna(s.median()))

# # 3) 在 test 中按 train 的位置中位数映射填补；若某位置在 train 未出现，则回退到全局中位数
# for col in phys_cols:
#     # 用 map 将 Position → 对应列的中位数
#     test[col] = test[col].fillna(test["Position"].map(pos_medians[col]))
#     # 若仍有缺失（位置不存在于 train），用 train 的整体中位数填补
...
# #力
# train['Power'] = round(train['Bench_Press_Reps'] * train['Weight'], 2)
# test['Power'] = round(test['Bench_Press_Reps'] * test['Weight'], 2)
...
submission.to_csv(PATH + '82_submission.csv', index=False)